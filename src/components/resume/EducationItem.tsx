...
const EducationItem = ({ qualification, institute, completionDate, description, qualifications }: { qualification: string, institute: string, completionDate: string, description?: string, qualifications?: string[] }) => {
  return <m.div className='flex flex-col gap-4 max-w-[400px]  flex-grow bg-zinc-700 px-6 py-4  border-2 border-slate-400 rounded-2xl' variants={animate}>
    <m.div className='flex flex-col gap-2'>
      <m.span className='flex flex-row justify-between'>
        ...
        <m.span className='rounded-lg p-1 px-3 bg-stone-800 text-slate-50 font-bold w-max'>{completionDate}</m.span>
      </m.span>
      ...
      <m.div className='flex flex-row flex-wrap gap-2'>
        {qualifications?.map(q => <m.span key={institute+q} className='rounded-full py-1 px-4 bg-stone-800 text-slate-50 font-bold w-max drop-shadow-lg flex-shrink '>{q}</m.span>)}
      </m.div>
    </m.div>
  </m.div>
}
