  ...
  return (
    <m.div id='about' className='min-w-full w-screen h-full min-h-screen overflow-scroll bg-gradient-to-l to-stone-900 from-stone-800 snap-center shrink-0 gap-4 px-8 no-scrollbar' ref={inViewRef} >
      <m.div className='flex justify-center py-8 flex-col items-center gap-2' >
        <m.h1 className='text-white text-7xl font-semibold whitespace-normal break-words text-center' initial={animateIn.initial} whileInView={{ ...animateIn.whileInView, transition: { delay: 0.55 } }} viewport={{ once: true, amount: 0.45 }} >
        ...
        <m.p className='text-white text-xl md:text-2xl font-light flex  justify-center gap-3 items-baseline relative whitespace-normal break-words' initial={animateIn.initial} whileInView={{ ...animateIn.whileInView, transition: { delay: 0.55 } }} viewport={{ once: true, amount: 0.45 }}>
          But you can call me <AnimatePresence>{nickName}</AnimatePresence>
        </m.p>
        <m.img src='./nomask.jpeg' className='w-80 h-80 cover object-cover mt-4 overflow-clip' style={{ borderRadius: '25% 8px 25% 8px', boxShadow: '4px 2px 12px 6px rgb(0,0,0,0.7)', }} initial={{ opacity: 0, y: 200, }} viewport={{ once: true, amount: 0.45 }} whileInView={{ opacity: 1, y: 0, transition: { delay: 1.1 } }} transition={{duration: 0.33,ease: 'easeInOut'}} whileHover={{ scale: 1.05, boxShadow: ' 0px 0px 12px 2px rgb(0,0,0,0.5)', borderRadius: '25% 25% 25% 25%' }} />
      </m.div>
